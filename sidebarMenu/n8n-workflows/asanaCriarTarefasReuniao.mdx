...
- **Validações**:
  ...
  - Valida o status das tarefas
  - Valida se realmente foi gerado um resumo
    - Caso as tarefas estiverem no processo de processamento no FireFiles, o fluxo aguarda 2 minutos e busca novamente os detalhes da transcrição até as terefas ficarem prontas.
  - Valida se o erro é de transcript not found
    - Por algum motivo as vezes o FireFiles envia o evento dizendo que a transcrição foi feita, mas ela não está disponível na busca da api ainda, nesse caso também cai no loop de aguardar 2 minutos e tentar buscar novamente.
...
- **AI Agent**: Analisa as tarefa e cria registros na tabela asana_tasks no Supabase
